...
/// <summary>
/// The compiled 13A registry (SLICE_S1_CONTRACT.md §6): every store S1 creates, registered against
/// EVERY closed purge class with a verb — NotApplicable is a registered exemption with a reason, never
/// a silent gap. This table is the "S1's own stores are the first registrants" clause: the CI gate that
/// fails an unregistered store lands with real, non-vacuous rows on day one.
/// </summary>
public sealed class PurgeRegistry : IPurgeRegistry
{
    public IReadOnlyList<PurgeRegistrationEntry> Entries { get; } = BuildEntries();

    public IReadOnlySet<string> RegisteredStoreKeys => _registeredStoreKeys ??= Entries.Select(e => e.StoreKey).ToHashSet();
    private IReadOnlySet<string>? _registeredStoreKeys;
