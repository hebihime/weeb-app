    ...
    IEventStore eventStore)
{
    ...
    public async Task<UserSearchExecutionOutcome> Execute(RequestContext callerCtx, UserSearchQueryClass queryClass, string term, string? cursor, CancellationToken ct = default)
    {
        ...
        var result = await executor.Execute(callerCtx, Action, target, reason: null, work: async workCtx =>
        {
            ...
            // for a query the cap itself blocks. ---
            ...
            {
                page = await searchSource.Search(new UserSearchQuery(queryClass, term, cursor), ct);
            }
            ...
            // comment) for this exact action key. ---
